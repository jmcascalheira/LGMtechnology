...
cores_tidy <- cores %>%
  ...
  mutate(CortexPerc = dplyr::recode(CortexPerc, "75-95%" = "76-100% Cortex", "25-75%" ="26-75% Cortex", "100%" = "76-100% Cortex", "<25%" = "1-25% Cortex", ">75%" = "76-100% Cortex", "0%" = "0% Cortex")) %>% 
...
elongated_tidy <- blanks %>%
  ...
  mutate(DorsalPattern = fct_lump(DorsalPattern, prop = 0.05)) %>%
  mutate(Termination = fct_lump(Termination, prop = 0.05)) %>%
  mutate(CortexPerc = dplyr::recode(CortexPerc, "75-95%" = "76-100% Cortex", "25-75%" ="26-75% Cortex", "100%" = "76-100% Cortex", "<25%" = "1-25% Cortex", ">95%" = "76-100% Cortex", "0%" = "0% Cortex")) %>%
  mutate(ScarNumber = as.factor(ScarNumber)) %>%
  mutate(DorsalPattern = dplyr::recode(DorsalPattern, "Unidentifiable" = "Other")) %>%
  mutate(ScarNumber = dplyr::recode(ScarNumber, "0" = "0 scars", "1" = "1 scar", "2" = "2 scars", "3" = "3 scars", "4" = "4 or more scars", "5" = "4 or more scars", "6" = "4 or more scars", "7" = "4 or more scars", "8" = "4 or more scars", "9" = "4 or more scars"))
...
In chronological terms, according to the traditional models [e.g.,  @jorda_solutrense_1955; @munoz_ibanez_puntas_2000; @tiffagom_pierre_2006; @zilhao_o_1997; @zilhao_seeing_2013; @zilhao_sequence_1994], backed shouldered projectiles and stemmed and winged points are only characteristic of the later phases of the Solutrean techno-complex (i.e., Upper Solutrean and Solutreo-Gravettian). However, this interpretation has been questioned in a recent Bayesian analysis of the currently available radiocarbon data [@cascalheiraChronologicalStructureSolutrean2015], calling into doubt the status of those type-fossils as precise temporal markers, and confirming their presence throughout most of the LGM time-span (i.e., c. 25-19 ka cal BP).

LGM settlement of southern Iberia is marked by the existence of more or less dense clusters of sites, that are relatively isolated from one another by geographical hiatus where the complete absence or the presence of very few sites might testify human passage but not likely intensive occupation [@aura_cueva_2009; @cascalheiraValeBoiAlgarve2012]. Caves and rock-shelters are better represented than open-air locations, as are multi-component sites over single horizon ones [@cascalheiraChronologicalStructureSolutrean2015; @cascalheira_influencia_2013]. There is evidence for the existence of different types of sites, including long-term and seasonal residential camps, shorter specialized occupations, lithic cache sites, and possible aggregation sites [sensu @conkey_identification_1980]. The number of occupied sites is significantly higher than during the early Upper Paleolithic and, although they tend to be in coastal or near-coastal settings, inland camps are also represented, mainly as single-occupation localities [@schmidtRapidClimateChange2012; @straus_upper_2000].

Rather problematic is the quality of the available data from most of these sites. More than half of the occurrences are surface or mixed contexts, suffering from several limitations (e.g., lack of organic materials, presence of small and/or highly truncated lithic assemblages, and occurrence of thick palimpsest deposits - [@aubry_analyse_2013; @cascalheiraChronologicalStructureSolutrean2015; @cascalheira_influencia_2013; @cascalheira_o_2013; @zilhao_seeing_2013]).

Probably as a result of this scenario, technological analyses of LGM lithic materials are (as well as faunal analysis) very limited in southern Iberia. These are mostly restricted to intra-site descriptions and evaluation of the assemblages [@aura_cueva_2009; @cascalheiraTecnologiaLiticaAbrigo2010; @cascalheiraValeBoiAlgarve2012; @ripoll_lopez_cueva_1988; @tiffagom_pierre_2006] and, very rarely, presenting inter-site or regional comparisons using technological patterns for the construction of clear paleoanthropological testable hypothesis [see @barton_glacial_2013 as an exception].
...
Both models seem to confirm the scarce data coming from faunal studies [@manneIntensiveSubsistencePractices2012; @yravedra_zooarqueologiy_2007] that demonstrate a seasonal movement of residential sites between coastal areas and the mountainous hinterland, and possibly a perennial occupation of few major sites across southern Iberia [see also @barton_glacial_2013].
...
For the three karstic contexts, a diachronic subdivision of the assemblages was also implemented based on the available lithostratigraphic information and previous analysis of the materials [@cascalheiraValeBoiAlgarve2012; @munoz_ibanez_puntas_2000; @ripoll_lopez_cueva_1988; @tiffagom_pierre_2006].  This subdivision resulted in a total of 10 contexts, representing, as much as possible, the chronological and geographical variability of LGM adaptations in southern Iberia.
...
Further, all four chosen sites are located near raw material sources, and thus all assemblages have been characterized as primary flaking assemblages, containing features such as a reasonable number and diversity of core maintenance pieces and large cortical flakes. A significant difference between sites is in the diversity of raw materials used. Although chert is always dominant, in Portuguese assemblages a greater diversity of materials is present. In the case of VB, non-siliceous raw materials were used not as alternatives to chert but for very specific functional purposes [@cascalheiraValeBoiAlgarve2012]. This pattern is consistent with that recorded for nearly all upper Paleolithic sites in the western Atlantic facade of Iberia even when good-quality chert is available within walking distance [@pereiraVariationQuartziteExploitation2013]. To avoid a possible impact on assemblage comparability caused by the use of different raw materials, only chert artifacts were selected for this study. This restriction was also imposed by the composition of the assemblages since for both Spanish sites it was not possible to identify more than a handful of artifacts made on other materials than chert, a fact that is also evident in previous publications for Ambrosio [@ripoll_lopez_cueva_1988].
...
total_blanks <- left_join(cores_prop, blanks_prop) %>% 
  ...
  mutate(`Blanks / Cores ratio` = Blanks / Cores) %>% 
...
```{r histogram, echo = FALSE, fig.cap = "Width histograms for the elongated products of each context included in this study."}
...
```{r intensity, echo = FALSE, fig.cap = "Density plots comparing the lengths of dominant blank scars on cores, blanks and fully cortical (primary) blanks. Following Henry [-@henry_correlations_1989] the plots show the relative flaking intensity for each assemblage."}
...
cols <- c("Cores" = "#E69F00", "Blanks" = "#56B4E9", "Primary Blanks" = "#ff2d2d")

ggplot() + 
  geom_density(data = core_length, aes(x=ScarLength, fill = "Cores"), color = "black", alpha = 0.7) +
  geom_density(data = blank_length, aes(x=Length, fill = "Blanks"), color = "black", alpha = 0.7) +
  geom_density(data = primary_blanks_length, aes(x=Length, fill = "Primary Blanks"), color = "black", alpha = 0.5) +
...
```{r MCAplatformmaintenance, echo=FALSE, fig.cap = "Multiple Correspondence Analysis plot (first two dimensions) for Platform Maintenance. The color ramp represents the contribution (in %) of the variable categories to the definition of the dimensions."}
...
platform_maintenance <- blanks_tidy %>%
  ...
  mutate(CortexPerc = dplyr::recode(CortexPerc, "76-100%" = "76-100% Cortex", "26-75%" ="26-75% Cortex", "100%" = "100% Cortex", "1-25%" = "1-25% Cortex", "0%" = "0% Cortex")) %>% 
  na.omit() %>%
  droplevels()
...
fviz_mca_var(mca_res_platform_maintenance, labelsize = 5, col.var = "contrib",
...
```{r ANOVAplatformmaintenance, echo=FALSE, fig.cap = "Platform Maintenance multi-comparison bootstrap ANOVA. Error bars represent 95% confidence interval from the mean. Contexts sharing the same set of letters do not present statistically significant difference (p-value ≥ 0.05)."}
...
```{r MCAcoreexploitation, echo=FALSE, fig.cap = "Multiple Correspondence Analysis plot (first two dimensions) for Core Exploitation. The color ramp represents the contribution (in %) of the variable categories to the definition of the dimensions."}
...
core_exploitation <- blanks_tidy %>%
  ...
  mutate(ScarNumber = as.factor(ScarNumber)) %>%
  mutate(ScarNumber = dplyr::recode(ScarNumber, "0" = "0 scars", "1" = "1 scar", "2" = "2 scars", "3" = "3 scars", "4" = "4 or more scars", "5" = "4 or more scars", "6" = "4 or more scars", "7" = "4 or more scars", "8" = "4 or more scars", "9" = "4 or more scars")) %>% 
  mutate(CortexPerc = dplyr::recode(CortexPerc, "76-100%" = "76-100% Cortex", "26-75%" ="26-75% Cortex", "1-25%" = "1-25% Cortex", "0%" = "0% Cortex")) %>% 
  mutate(DorsalPattern = dplyr::recode(DorsalPattern, "Unidentifiable" = "Other")) %>%
  na.omit() %>%
  droplevels()
...
fviz_mca_var(mca_res_core_exploitation, labelsize = 5, col.var = "contrib",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
             repel = TRUE, invisible = "quali.sup",
             ggtheme = theme_gray(), title = "") +
  theme(axis.text.x = element_text(size = 14),
...
```{r ANOVAcoreexploitation, echo=FALSE, fig.cap = "Core Exploitation multi-comparison bootstrap ANOVA. Error bars represent 95% confidence interval from the mean. Contexts sharing the same set of letters do not present statistically significant difference (p-value ≥ 0.05)."}
...
```{r MCAconvexity, echo=FALSE, fig.cap = "Multiple Correspondence Analysis plot (first two dimensions) for Dorsal Surface Convexity. The color ramp represents the contribution (in %) of the variable categories to the definition of the dimensions."}
...
fviz_mca_var(mca_res_dorsal_convexity, labelsize = 4, col.var = "contrib",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
             repel = TRUE, invisible = "quali.sup",
             ggtheme = theme_gray(), title = "") +
  theme(axis.text.x = element_text(size = 14),
...
```{r ANOVAconvexity, echo=FALSE, fig.cap = "Dorsal Convexity multi-comparison bootstrap ANOVA. Error bars represent 95% confidence interval from the mean. Contexts sharing the same set of letters do not present statistically significant difference (p-value ≥ 0.05)."}
...
```{r MCAelongated, echo=FALSE, fig.cap = "Multiple Correspondence Analysis plot (first two dimensions) for Elongated Products. The color ramp represents the contribution (in %) of the variable categories to the definition of the dimensions"}

# Filter specific categories for MCA analysis
elongated <- elongated_tidy %>%
  filter(CortexPerc != "76-100% Cortex") %>% 
  filter(ScarNumber != "0 scars") %>%
...
fviz_mca_var(mca_res_elongated, labelsize = 5, col.var = "contrib",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
             repel = TRUE, invisible = "quali.sup",
             ggtheme = theme_gray(), title = "") +
  theme(axis.text.x = element_text(size = 14),
...
```{r ANOVAelongated, echo=FALSE, fig.cap = "Elongated Products multi-comparison bootstrap ANOVA. Error bars represent 95% confidence interval from the mean. Contexts sharing the same set of letters do not present statistically significant difference (p-value ≥ 0.05)."}
...
```{r MCAcoremorphology, echo=FALSE, fig.cap = "Multiple Correspondence Analysis plot (first two dimensions) for Core Morphology. The color ramp represents the contribution (in %) of the variable categories to the definition of the dimensions"}
...
fviz_mca_var(mca_res_cores, labelsize = 5, col.var = "contrib",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
             repel = TRUE, invisible = "quali.sup",
             ggtheme = theme_gray(), title = "") +
  theme(axis.text.x = element_text(size = 14),
...
```{r ANOVAcoremorphology, echo=FALSE, fig.cap = "Core Morphology multi-comparison bootstrap ANOVA. Error bars represent 95% confidence interval from the mean. Contexts sharing the same set of letters do not present statistically significant difference (p-value ≥ 0.05)."}
...
```{r MCAcoreuse, echo=FALSE, fig.cap = "Multiple Correspondence Analysis plot (first two dimensions) for Core Use. The color ramp represents the contribution (in %) of the variable categories to the definition of the dimensions"}
...
fviz_mca_var(mca_res_core_use, labelsize = 5, col.var = "contrib",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
             repel = TRUE, invisible = "quali.sup",
             ggtheme = theme_gray(), title = "") +
  theme(axis.text.x = element_text(size = 14),
...
```{r ANOVAcoreuse, echo=FALSE, fig.cap = "Core Use multi-comparison bootstrap ANOVA. Error bars represent 95% confidence interval from the mean. Contexts sharing the same set of letters do not present statistically significant difference (p-value ≥ 0.05)."}
...
```{r ANOVAsimilarity, echo = FALSE, fig.cap= "Heat map summarizing the number of non-significantly different bootstrapped ANOVA tests between each pair of assemblages. Letters inside each square indicate the heuristic domains responsible for the similarities between assemblages: A - Platform Maintenance; B - Direction of Core Exploitation; C - Dorsal Surface Convexity; D - Elongated blanks techno-morphology; E - Core Morphology; F - Core Use. For more information on each of the contexts please refer to Figure 1 and Table 2."}
...
# geom_text(aes(Var1, Var2, label = value), color = "black", size = 4) +
 ...
 coord_fixed() +
 geom_text(aes(label = tests), size = 3)
...
Finally, it is also noteworthy that, despite the similarities in some technological aspects, all VB contexts tend to appear isolated from the remaining contexts used in this study. In previous studies, technological idiosyncrasies of VB have been noted and are not exclusive for LGM occupations [see e.g., @marreirosLithicTechnologyGravettian2015]. Possible explanations are related with the low knapping quality of chert, featuring small nodules and volumes highly fractured by tectonics [@pereiraTerritoryAbioticResources2016], available within the VB raw materials acquisition territory. This most likely led hunter-gatherers to develop specific technological strategies fitted to the characteristics of the raw materials [for further debate on the influence of raw materials in technological strategies see @andrefsky_jr_raw-material_1994]. To some degree, VB technological options are stable across the site's sequence, and independent of changes introduced in tools shape at the beginning of each Upper Paleolithic techno-complex [@cascalheiraCrossscaleAdaptiveBehaviors2017]. In the particular case of the Solutrean and the hypotheses raised by this study, technological practices transmission might have occurred in contexts of a high degree of social intimacy, but regional ecological constraints did not allow the generalization of basic technological patterns.

As in any other geographies and timeframes, technological variation over the LGM in southern Iberia was the result of a complex relationship among patterns of inheritance, interaction, and regional adaptation. Population interaction across larger territories was certainly a key factor of the adaptive response to the impact of climate and landscape change and seem to have functioned through sharing behaviors of stylistic/typological concepts, but also of technological solutions. Concept sharing could have occurred across regions in different acculturation environments, depending on the geographical distance between communities and preferential cultural ties, but also on the impact that local resources could have had in the technological decisions. After all, humans, as other biota, interact with the environment at distinct scales and create self-reinforcing adaptive patterns [@sundstromTransdisciplinaryApplicationCrossscale2014]. The multiple but distinct scales of self-organization during the LGM in southern Iberia, and the distribution of behaviors within and across scales might have been one of the most important generators of resilience for those communities [@cascalheiraCrossscaleAdaptiveBehaviors2017].

Only further studies using more data from the regions addressed in this study, and other territories located between them, such as the Alentejo, southwestern Spain, or Central Iberia [@alcaraz-castanoHumanSettlementCentral2017], will allow the ascertainment of the extension of the several scales of influence and contacts.

\n

# Acknowledgments

Financial support for the analysis of the materials included in this study was provided by the Fundação para a Ciência e Tecnologia (FCT) (grant SFRH/BD/65527/2009) and by a STSM COST action (ref. COST-STSM-TD0902-10855). The author is funded by FCT, contract ref. DL 57/2016/CP1361/CT0026. Work at Vale Boi is funded by the project ALG-01-0145-FEDER-27833 - PTDC/HAR-ARQ/27833/2017. For permission to analyze the materials I would like to thank the Servicio de Investigación Prehistórica de Valencia at the Museu de Prehistoria de Valencia, the Museo Arqueológico Provincial de Almería, and the Museu Nacional de Arqueologia in Portugal. The author would like to thank to Dr. Manuel Alcaraz-Castaño for the insightful comments on the paper, as well as to the anonymous reviewer whose suggestions really helped to improve the quality of the final version of the article.
