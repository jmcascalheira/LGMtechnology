...
For its uniqueness and rough coincidence with the LGM, the Solutrean record often emerges as a promising context for examining the mechanisms of formal and chronological change of lithic tools during the late Pleistocene. Further, it has been recognized as a great opportunity to test the possible links between the development of specific technical innovations and social behavior under a particular climate setting [@derrico_limpact_2006; @renard_continuity_2011; @villaverde_upper_1998].
...
Contacts occurring anywhere between these two extremes of the contact scale result in dissimilar access to information and to different types of information.
...
knitr::kable(total_blanks, digits = 2)
...
As it is shown in Figure \@ref(fig:elongated_histogram), however, the reduction sequences for elongated blanks seem to have followed an unimodal tendency in all assemblages. No clear separation between blade and bladelet production is attested within contexts and thus, as in other studies [see e.g. @bicho_technological_1992], the traditional subdivision [as presented by @tixier_typologie_1963] was not used in the statistical tests below, where both classes were grouped as a single elongated blanks category.


```{r elongated_histogram, echo = FALSE, fig.cap = "Width histograms for the elongated products from each context included in this study."}
...
All these patterns seem to corroborate an overall broad comparability of the assemblages. As expected, some reservations should be taken regarding the site of VALM, which seems, in the first instance, to present some technological idiosyncrasies. These are mostly related with a lower blank to core ratio, the fact that the cores seem to be more reduced than expected, and the dimensions of the elongated blanks are amongst the largest of all the studied assemblages. Two reasons may be behind this pattern: either some of the late production blanks were actually exported from the site, or the methods used during the excavation of the site (in the late 1940s, the early 1950s) did not allow the recovery of smaller materials. This latter hypothesis is perhaps the most parsimonious, given the small amount of chips available from the site [@zilhao_o_1997].

In spite of the particularities mentioned above, the overall assemblage compositions are identical. Together with the fact that all assemblages are considered primary flaking sites located close to raw materials sources, and that all have been previously classified as residential contexts, similarities in flaking intensities and reduction tendencies somehow guarantee a higher degree of certainty in the comparisons made by the tests presented below.
...
The analysis of platform maintenance aimed to explore how knappers manage platform characteristics to control blank dimensions and morphology across the reduction sequences. This approach was accomplished by investigating the relationship between platform type, dorsal cortex percentage, and blank elongation (length/width ratio). As in Scerri [-@scerri_unexpected_2014], the external platform angle measurements were not available for this analysis. Taking into account, however, the high degree of correlation between that variable and blank elongation [@pelcin_controlled_1996], the latter was used as a proxy for possible changes in the management of the external platform angle. The results from the application of a MCA to this set of variables are presented in Figure @ref(fig:MCA_platform_maintenance) and Table @ref(tab:platform_maintenance_description).
...
```{r MCA_platform_maintenance, echo=FALSE, fig.cap = "Multiple Correspondence Analysis plot for Platform Maintenance. For interpretation of the references to colour in this figure legend, the reader is referred to the web version of this article."}
...
fviz_mca_var(mca_res_platform_maintenance, col.var = "contrib",
...
```{r platform_maintenance_description, echo = FALSE}

var_description_platform_maintenance <- data.frame(
  Dimension = c("Dimension 1", "Dimension 2"),
  Variability = c("26%", "13.6%"),
  Positive = c("Cortical platforms; Low elongation; 76-100% cortex", "Cortical and Crushed platforms; 76-100% cortex"),
  Negative = c("Crushed platforms; High elongation; 0% cortex",
  "Faceted and Plain platforms; 26-75% cortex"),
  Interpretation = c("Different types of platforms associated with different moments of the the reduction sequence","Management of platforms through faceting in different phases of the reduction sequence"))

knitr::kable(var_description_platform_maintenance, caption = "First two components of the Multiple Correspondence Analysis for the Platform Maintenance domain.")
...
```{r ANOVA_platform_maintenance, echo=FALSE, fig.cap = "Platform Maintenance multi-comparison bootstrap ANOVA. Error bars represent 95% confidence interval from the mean. Contexts sharing the same set of letters do not present statistically significan difference."}
...
The analysis of core exploitation using blanks aimed to characterize the way cores were rotated by knappers to create different dorsal surface convexity patterns during the sequences of reduction. A combination of dorsal cortex frequency, dorsal pattern, and number of scars in the dorsal surface was used in the MCA. Results are presented in Figure \@ref(fig:MCA_core_exploitation) and Table \@ref(tab:core_exploitation_description).
...
fviz_mca_var(mca_res_core_exploitation, col.var = "contrib",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
             repel = TRUE, invisible = "quali.sup",
             ggtheme = theme_gray(), title = "")
...
```{r core_expoitation_description}

var_description_core_exploitation <- data.frame(
  Dimension = c("Dimension 1", "Dimension 2"),
  Variability = c("38.5%", "23.2%"),
  Positive = c("76-100% cortex; Other strategies; 0 dorsal scars", "0% cortex; 4 or more dorsal scars; Bidirectional"),
  Negative = c("0% and 26-75% cortex; Bidirectional and Unidirectional strategies", "26-75% cortex; Unidirectional strategies; 1 or 2 scars"))

kable(var_description_core_exploitation, caption = "First two components of the Multiple Correspondence Analysis for the Core Exploitation domain.")

```

\n

```{r ANOVA_core_exploitation, echo=FALSE, fig.cap = "Core Exploitation multi-comparison bootstrap ANOVA. Error bars represent 95% confidence interval from the mean. Contexts sharing the same set of letters do not present statistically significan difference."}
...
fviz_mca_var(mca_res_dorsal_convexity, col.var = "contrib",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
             repel = TRUE, invisible = "quali.sup",
             ggtheme = theme_gray(), title = "")
...
```{r dorsal_convexity, echo = FALSE}

var_description_core_exploitation <- data.frame(
  Dimension = c("Dimension 1", "Dimension 2"),
  Variability = c("22.2%", "10.2%"),
  Positive = c("Elongation low; Flattening high; Cross-section lenticular", "Edge shape irregular; Profile irregular; Cross-section irregular"),
  Negative = c("Elongation high; Edge shape parallel; Cross-section trapezoidal", "Flattening high; Cross-section lenticular"))

kable(var_description_core_exploitation, caption = "First two components of the Multiple Correspondence Analysis for the Dorsal Surface Convexity domain.")

```



```{r ANOVA_dorsal_convexity, echo=FALSE, fig.cap = "Dorsal Convexity multi-comparison bootstrap ANOVA. Error bars represent 95% confidence interval from the mean. Contexts sharing the same set of letters do not present statistically significan difference."}
...
```{r MCA_elongated, echo=FALSE, fig.cap = "Multiple Correspondence Analysis plot for Elongated Products. For interpretation of the references to colour in this figure legend, the reader is referred to the web version of this article."}
...
fviz_mca_var(mca_res_elongated, col.var = "contrib",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
             repel = TRUE, #invisible = "quali.sup",
             ggtheme = theme_gray(), title = "")
...
```{r elongated_description}

var_description_elongated <- data.frame(
  Dimension = c("Dimension 1", "Dimension 2"),
  Variability = c("24.2%", "11.2%"),
  Positive = c(""),
  Negative = c(""),
  Interpretation = c("", "")
)

kable(var_description_elongated, caption = "First two components of the Multiple Correspondence Analysis for the Elongated Products domain.")

```

\n

```{r ANOVA_elongated, echo=FALSE, fig.cap = "Elongated Products multi-comparison bootstrap ANOVA. Error bars represent 95% confidence interval from the mean. Contexts sharing the same set of letters do not present statistically significan difference."}
...
```{r MCA_core_morphology, echo=FALSE, fig.cap = "Multiple Correspondence Analysis plot for Core Morphology. For interpretation of the references to colour in this figure legend, the reader is referred to the web version of this article."}
...
```{r core_morphology, echo = FALSE}

var_description_core_morphology <- data.frame(
  Dimension = c("Dimension 1", "Dimension 2"),
  Variability = c("26.3%", "22.5%"),
  Positive = c("",
    ""),
  Negative = c("",
  ""
...
kable(var_description_core_morphology, caption = "First two components of the Multiple Correspondence Analysis for the Core Morphology domain.")

```


```{r ANOVA_core_morphology, echo=FALSE, fig.cap = "Core Morphology multi-comparison bootstrap ANOVA. Error bars represent 95% confidence interval from the mean. Contexts sharing the same set of letters do not present statistically significan difference."}
...
dim1 <- ggplot(mca_summary) +
  geom_point(stat = "identity", aes(x = context, y = meandim1)) +
  geom_errorbar(stat = "identity", aes(x = context, ymin = LCIdim1, ymax = MCIdim1), width = 0) +
  geom_hline(aes(yintercept = 0), color = "red", linetype = "dashed", show.legend = FALSE) +
  geom_text(data = grouping, aes(x = context, y = m1 + 0.08, label = Lettersdim1, color = Lettersdim1), show.legend = FALSE) +
  xlab("") +
  ylab("Dimension 1") +
  scale_colour_discrete(l = 40) +
  coord_flip()
...
dim2 <- ggplot(mca_summary) +
  geom_point(stat = "identity", aes(x = context, y = meandim2)) +
  geom_errorbar(stat = "identity", aes(x = context, ymin = LCIdim2, ymax = MCIdim2), width = 0) +
  geom_hline(aes(yintercept = 0), color = "red", linetype = "dashed", show.legend = FALSE) +
  geom_text(data = grouping, aes(x = context, y = m2 + 0.08, label = Lettersdim2, color = Lettersdim2), show.legend = FALSE) +
  xlab("") +
  ylab("Dimension 2") +
  scale_colour_discrete(l = 40) +
  coord_flip()
...
```{r MCA_core_use, echo=FALSE, fig.cap = "Multiple Correspondence Analysis plot for Core Use. For interpretation of the references to colour in this figure legend, the reader is referred to the web version of this article."}
...
fviz_mca_var(mca_res_core_use, col.var = "contrib",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
             repel = TRUE, invisible = "quali.sup",
             ggtheme = theme_gray())
...
```{r core_use, echo = FALSE}

var_description_core_use <- data.frame(
  Dimension = c("Dimension 1", "Dimension 2"),
  Variability = c("22.1%", "12.3%"),
  Positive = c("Heavy cores used for flake extraction using two isolated or multiple, often cortical, platforms",
    "Cores with more than two platforms for the extraction of mixed products"),
  Negative = c("Highly elongated cores for the extraction of elongated products using two opposed, often faceted, platforms",
  "Extraction of elongated products from single platform cores",
  Interpretation = c("","")))

kable(var_description_core_use, caption = "First two components of the Multiple Correspondence Analysis for the Core Use domain.")

```


```{r ANOVA_core_use, echo=FALSE, fig.cap = "Core Use multi-comparison bootstrap ANOVA. Error bars represent 95% confidence interval from the mean. Contexts sharing the same set of letters do not present statistically significan difference."}
...
# Conclusions
The extent by which human culture varies over space and time is the result of a complex interplay among patterns of inheritance, interaction, and local adaptation. Distinguishing how these three types of processes generate observed patterns of cultural variation is one of the primary research questions in archaeology and anthropology.

Many factors could promote the formation of such clusters, including assortativity by cultural similarity, or conformity to local norms of, for example, tool production.


# Acknowledgments

Financial support for the analysis of the materials included in this study was provided by the Portuguese National Science Foundation (FCT) (grant SFRH/BD/65527/2009) and by a STSM COST action (ref. COST-STSM-TD0902-10855). For permission to analyze the materials I would like to thank the Servicio de Investigación Prehistórica de Valencia at the Museu de Prehistoria de Valencia, the Museo Arqueológico Provincial de Almería, and the Museu Nacional de Arqueologia in Portugal. I would like also to acknowledge Nuno Bicho for the useful comments that helped to improve early versions of the text, as well as Brandon Zinsious for the english revision of the initial manuscript.
